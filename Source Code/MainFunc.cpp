...
int main() {
	...
	bool swdel1 = 1;					// value = 1 if deleting scenario 1 is activated, 0 otherwise (Sec. 2.1.2 in the manuscript)
	bool swdel2 = 1;					// value = 1 if deleting scenario 2 is activated, 0 otherwise (Sec. 2.1.2 in the manuscript)
	...
	//-------------Neighbour search algorithm using moving window (Sec 2.1.1)-----------------//
	if (neighbor_input == "NULL") {
	...
	for (int i = 0; i < n_part; i++) {
		...
		if (NC == -1) {    // if NC==-1, then no NC will be used, rather only R_c will be used as cut-off
			range = R_c - delr / 2.0;;
		}
		for (int j = 0; j < RcPos[i].size(); j++) {
			Real r = dist(pos.at(i), pos.at(RcPos[i][j]));
	...
	//-----------------Deleting overlapping particle edge-connections (Deleting scenario 1 in Sec 2.1.2)--------------//
	int Corrcount = 0;
	if (swdel1 == 1) {
		for (int i = 0; i < n_part; i++) {
			...
			for (int j = 0; j < i_size; j++) {
				...
				Real testangle;
				Real testangletest;
				// Algorithm 4 in the paper is implemented in the next part
				for (int j = 0; j < deletelist.size(); j++) {
					...
					resultangles = angle(ot, on);     //----Angle between two vectors - two outputs - first-> smallest angle(regardless of direction) ;  second-> smallest positive angle(0,PI)----//
				...
				// Finding on which side each neighboring  particle belong to
				for (int j = 0; j < deletelist.size(); j++) {
					int n_ind = deletelist[j];
					if (n_ind != test_n) {
				...
				// Building the list of deletable edges that may cause overlaps
	...
	// Sorting the list of deletable edges and sequentially deleting edges according to description in Sec. 2.1.2 
	...
	// Deleting scenario 2 is implemented in the next part (Sec 2.1.2) 
	if (swdel2 == 1) {
		for (int i = 0; i < n_part; i++) {
	...
	//--------------------Algorithm 5 in the paper is implemented in the next part--------------//
	...
	//--------------------Algorithm 6 in the paper is implemented in the next part--------------//
	map<int, bool>worked;						
		...
		if (single_conn.at(workingnow).size() >= 2) {
			if ((worked[workingnow] == 0 && single_conn.at(workingnow).size() >= 2) || (worked[workingnow] == 1 && single_conn.at(workingnow).size() > 2)) {
				...
				while (next != workingnow && stop == 0) {
					...
					else if (single_conn.at(next).size() == 2) {			// This condition is visited for a case similar to Fig 7(b)
					...
					else {													// This condition is visited for a case similar to Fig 8(b)
						int nextdirect = 0;
						vector<int>finallist;
	...
	//--------------------Algorithm 7 in the paper is implemented in the next part--------------//
	if (swhole == 1) {
					...
					if (currentsize > 2 && pos_neighbor[adjacentnode].size() > 2) {				
						...
						if (currentfix != currentnode_fix && vector_search(pos_neighbor.at(currentnode_fix), currentfix) == 0)
						{
							if (dist(pos.at(currentnode_fix), pos.at(currentfix)) < 1.0 * R_c) {
								// Checking if any new overlap is being created due to creating a new connection
	...
	//--------------------Finding empty regions (Sec 2.1.4)--------------------------//
	...
	//--------- Finding capsomer positions for locations of empty nodes (Sec 2.1.4)----------------//
	
	if (sweempty == 1) {
		for (int q = 0; q < empty_connection.size(); q++) {
			int emptynode1 = empty_connection[q].first;
			int emptynode2 = empty_connection[q].second;
			if (emptynode1 > nonrepeat_part && emptynode2 > nonrepeat_part)continue;     // If no particles in the pair are part of the non repeated particles (original particles) then we do not need to calculate for that pair
			int i = emptynode1;
