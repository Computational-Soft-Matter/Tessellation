...
int main() {
	string input_file = "sp10exp";								// File contains the 3D coordinates of the colloidal particles
	...
	if (neighbor_input == "NULL") {
		for (int i = 0; i < n_part; i++) {
			...
			cout << "Search completed for particle: " << i << endl;
			...
			for (int l = 0; l < 2; l++) {
				...
				if (commoncount == 1 && currentsize > 2 && pos_neighbor[adjacentnode].size() > 2) {			// if fixing node is still a single edged connection then keep going for fix	
					...
					for (int mm = 1; mm < fixes[indnow].size(); mm++) {
						...
						{
							if (dist(pos.at(currentnode_fix), pos.at(currentfix)) < 1.5 * R_c) {
